...
def zigzag_pivots(close: pd.Series, threshold: float, label: int) -> pd.Series:
    ...
    Args:
        ...
        label: Target label (1=peak, -1=valley).

    Returns:
        Boolean series where True indicates a pivot matching the label.
    """
...
def add_features_large(pset: gp.PrimitiveSetTyped) -> None:
    ...
    for name, cls in [
        ...
        try:
            pset.addEphemeralConstant(name, cls.sample, cls)
        except Exception:
            pass
...
def create_pset_default_medium(name: str = "default_medium") -> gp.PrimitiveSetTyped:
    """Create medium sized pset with default features (no zigzag)."""
    pset = create_pset_core(name)
    add_features_medium(pset)
    return pset
...
def create_pset_default_large(name: str = "default_large") -> gp.PrimitiveSetTyped:
    """Create large pset with default features (no zigzag)."""
    pset = create_pset_core(name)
    add_features_large(pset)
    return pset
