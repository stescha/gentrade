...
from gentrade.config import BacktestConfig, TREE_GEN_FUNCS, RunConfig
...
def evaluate(
    ...
    Args:
        ...
        fitness_fn: Callable ``(y_true, y_pred) -> float``. Typically a
            ``ClassificationFitnessConfigBase`` subclass instance.
...
def create_toolbox(cfg: RunConfig, pset: gp.PrimitiveSetTyped) -> base.Toolbox:
    ...
    if mut._requires_expr:
        ...
        extra_kwargs: dict[str, Any] = {"expr": toolbox.expr_mut}
    else:
        extra_kwargs = {}

    if mut._requires_pset:
        extra_kwargs["pset"] = pset

    mut_params = {k: v for k, v in mut.params.items() if k not in {"expr_min_depth", "expr_max_depth"}}
    toolbox.register("mutate", mut.func, **mut_params, **extra_kwargs)

    # Bloat control — staticLimit decorates both mate and mutate
    toolbox.decorate(
        "mate", gp.staticLimit(key=operator.attrgetter("height"), max_value=cfg.tree.max_height)
    )
    toolbox.decorate(
        "mutate", gp.staticLimit(key=operator.attrgetter("height"), max_value=cfg.tree.max_height)
    )
...
def run_evolution(
    ...
    # ── 6. Evaluation dispatch via _requires_backtest flag ─
