...
class FitnessConfigBase(_ComponentConfig):
    ...
    - ``__call__`` is a one-line delegation to the underlying function or
      fitness class.
    - ``_requires_backtest``: if ``True``, the caller dispatches to the
      backtest evaluation path instead of the classification path.
    """
...
class RunConfig(BaseModel):
    ...
    backtest: BacktestConfig | None = Field(
        None,
        description="Backtest parameters; required when using a backtest fitness",
    )
