...
class TestPairEvaluatorComputation:
    ...
    def test_buy_aggregation_returns_float_tuple(
        ...
        ev = PairEvaluator(pset=pset, metrics=(F1Metric(tree_aggregation="buy"),))
        result = ev.evaluate(pair_individual, ohlcvs=[df], entry_labels=[entry_labels])
    ...
    def test_sell_aggregation_returns_float_tuple(
        ...
        ev = PairEvaluator(pset=pset, metrics=(F1Metric(tree_aggregation="sell"),))
        result = ev.evaluate(pair_individual, ohlcvs=[df], exit_labels=[exit_labels])
        assert isinstance(result, tuple)
        assert len(result) == 1
        assert np.isfinite(result[0])
    ...
    def test_const_metric_buy_aggregation(
        ...
        ev = PairEvaluator(pset=pset, metrics=(_ConstMetric(tree_aggregation="buy"),))
        result = ev.evaluate(pair_individual, ohlcvs=[df], entry_labels=[entry_labels])
    ...
    def test_multi_dataset_averages(
        ...
        ev = PairEvaluator(pset=pset, metrics=(_CountMetric(tree_aggregation="buy"),))
        result = ev.evaluate(
            pair_individual,
            ohlcvs=[df, df],
